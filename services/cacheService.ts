...
import type { ClubResponse, ClubEvent, ClubPost } from '@/types/club';
...
export const clubDetailCache: Record<string, ClubResponse> = {};
export const clubPostsCache: Record<string, ClubPost[]> = {};
export const clubEventsCache: Record<string, ClubEvent[]> = {};
...
export const CacheService = {
  ...
  clearAll: () => {
    ...
    for (const key of Object.keys(clubDetailCache)) {
      delete clubDetailCache[key];
    }
    for (const key of Object.keys(clubPostsCache)) {
      delete clubPostsCache[key];
    }
    for (const key of Object.keys(clubEventsCache)) {
      delete clubEventsCache[key];
    }
  },
};
